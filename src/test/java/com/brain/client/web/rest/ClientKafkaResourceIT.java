package com.brain.client.web.rest;
...
import com.brain.client.config.KafkaProperties;
...
class ClientKafkaResourceIT {
    ...
    void setup() {
        ...
        ClientKafkaResource kafkaResource = new ClientKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/client-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/client-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
